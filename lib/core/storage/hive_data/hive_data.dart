...
import 'package:kickin_storage/kickin_storage.dart' as ks;
import 'package:slidesync/core/utils/result.dart';

class KVStore {
  static final _app = ks.AppHive();
  static final me = const KVStore._();
  const KVStore._();

  Future<void> initialize() async => await _app.initialize();

  T? getData<T>({required String key}) =>
      Result.tryRun(() => _app.getData(key: key)).data as T?;

  Future<void> setData<T>({required String key, required T? value}) async =>
      await _app.setData(key: key, value: value).tryRunAsync();

  Future<void> deleteData({required String key}) async =>
      await _app.deleteData(key: key).tryRunAsync();

  Future<bool> resetAll(String acknowledge) => Result.tryRunAsync(() async {
    await _app.resetAll(acknowledge);
  }).then((_) => true).catchError((_) => false);
}

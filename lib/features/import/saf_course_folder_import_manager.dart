...
//   /// Pick a folder using SAF and cache it
//   static Future<String?> pickFolder() async {
//     try {
//       log('📂 Opening SAF folder picker...');

//       // Create a temporary path for SAF to use
//       final tempPath = p.join(Directory.systemTemp.path, 'saf_${DateTime.now().millisecondsSinceEpoch}');
//       final saf = Saf(tempPath);

//       // Request directory access using SAF
//       final bool? isGranted = await saf.getDirectoryPermission(isDynamic: false);

//       if (isGranted == null || !isGranted) {
//         log('❌ No folder selected or permission denied');
//         return null;
//       }

//       log('✅ Folder access granted');

//       // Cache the files using SAF
//       final bool? isCached = await saf.sync();

//       if (isCached == null || !isCached) {
//         log('❌ Failed to cache folder');
//         return null;
//       }

//       log('✅ Folder cached successfully');

//       // Get cached files paths
//       final List<String>? cachedPaths = await saf.getCachedFilesPath();

//       if (cachedPaths == null || cachedPaths.isEmpty) {
//         log('⚠️ No files found in cached folder');
//         // Return a placeholder path since we have permission
//         return tempPath;
//       }

//       // Return the parent directory of the first cached file
//       final firstFilePath = cachedPaths.first;
//       final parentPath = Directory(firstFilePath).parent.path;

//       log('📍 Selected folder path: $parentPath');
//       return parentPath;
//     } catch (e, st) {
//       log('❌ Error picking folder: $e', stackTrace: st);
//       return null;
//     }
//   }
...
//   /// Scan a folder using traditional file system (SAF just caches, doesn't traverse)
...
//             log('   └─ Is File: ${p.basename(entity.path)}');
...
//     final Directory tempDir = Directory.systemTemp.createTempSync('course_import_');
...
//         final copiedFile = File(p.join(tempDir.path, newFileName));

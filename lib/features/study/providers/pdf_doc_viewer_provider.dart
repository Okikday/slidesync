...
class PdfDocViewerProvider {
  static final state = Provider.family.autoDispose<PdfDocViewerState, String>((
    ref,
    arg,
  ) {
  ...
  static final searchState = Provider.family
      .autoDispose<PdfDocSearchState, String>((ref, arg) {
        final controller = ref.read(state(arg).select((s) => s.controller));
        final pdss = PdfDocSearchState(
          contentId: arg,
  ...
  static final ispdfViewerInDarkMode =
      AsyncNotifierProvider.autoDispose<ToggleNotifier, bool>(
        () => ToggleNotifier(HiveDataKey.ispdfViewerInDarkMode.name),
      );
}

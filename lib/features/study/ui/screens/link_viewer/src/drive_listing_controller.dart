...
class DriveListingController {
  ...
  Future<void> importAll(drive_service.DriveResource resource) async {
    ...
    try {
      final files = (resource.children ?? const <drive_service.DriveFile>[])
      ...
      final outcome = await _importDriveSelection(
        resource: resource,
        selectedFiles: files,
        apiKey: apiKey,
      );
  ...
  Future<void> showFileOpenOptions(
    ...
    required void Function(String folderId, {String? folderName})
    onNavigateToFolder,
  }) async {
    if (file.isFolderLike) {
      ...
      if (targetFolderId == null) {
        log(
          'DriveListingView: folder-like item ${file.name ?? 'unnamed item'} had no navigation target',
        );
        return;
      }
      ...
      onNavigateToFolder(
        targetFolderId,
        folderName: file.name ?? targetFolderId,
      );
      return;
    }
    ...
    final actions = <AppActionDialogModel>[
      ...
      AppActionDialogModel(
        ...
        onTap: () async {
          ...
          await _openInApp(
            file,
            preResolvedFile: displayFile,
            preferredKind: inAppKind,
          );
        },
      ),
    ];
  ...
  Future<DriveImportOutcome> _importDriveSelection({
    ...
    final destination = await _resolveOrCreateImportCollection(
      course.uid,
      collectionTitle,
    );
    if (destination == null) {
    ...
    if (cachedEntries.isEmpty) {
      return DriveImportOutcome(
        ...
        itemCount: 0,
        totalBytes: 0,
        note: encounteredNotes.isEmpty
            ? 'Nothing could be cached'
            : encounteredNotes.join(' • '),
      );
    }
    ...
    try {
      ...
      final successCount = addResults
          .where((result) => result.isSuccess)
          .length;
      final storedBytes = addResults
          .where((result) => result.isSuccess)
          .fold<int>(0, (sum, result) => sum + (result.fileSize ?? 0));
      final duplicateCount = addResults
          .where((result) => result.hasDuplicate)
          .length;
      ...
      await _annotateImportedDriveContents(
      ...
      if (duplicateCount > 0) {
        notes.add(
          '$duplicateCount duplicate${duplicateCount == 1 ? '' : 's'} already existed in the library',
        );
      }
      ...
      for (final entry in cachedEntries.where(
        (entry) => !entry.reusedExistingPath,
      )) {
        ...
        try {
          ...
          log(
            'DriveListingView: failed to delete cached file ${entry.path}',
            error: error,
  ...
  Future<void> _recordImportedDownloadHistory({
    ...
    final itemCount = entries.length < addResults.length
        ? entries.length
        : addResults.length;
    ...
    for (var i = 0; i < itemCount; i++) {
      ...
      await historyNotifier.addEntry(
        DownloadHistoryEntry(
          ...
          note: entry.reusedExistingPath
              ? 'Opened from existing local copy'
              : 'Imported from Drive',
        ),
      );
    }
  ...
  Future<DriveCacheEntry> _downloadAndCacheDriveFileEntry({
    ...
    if (effectiveFile == null) {
      ...
      return DriveCacheEntry(
        ...
        bytes: 0,
        note:
            'Skipped $fileLabel because its shortcut target could not be resolved',
        sourceFile: file,
        fingerprint: DriveSourceFingerprint.fromFile(file),
      );
    }
    ...
    final existingContent = await _findExistingDriveContent(
      effectiveFile,
      existingContents,
    );
    ...
    final transferId =
        'drive-${effectiveFile.id}-${DateTime.now().microsecondsSinceEpoch}';
    ...
    try {
      ...
      final bytes = await _downloadDriveBytes(
        ...
        onProgress: (receivedBytes, totalBytes) {
          ...
          final progress = resolvedTotalBytes > 0
              ? receivedBytes / resolvedTotalBytes
              : 0.0;
          ...
          if (receivedBytes == resolvedTotalBytes ||
              receivedBytes == 0 ||
              receivedBytes % (1024 * 512) == 0) {
            downloadFeedNotifier.updateProgress(
              ...
              totalBytes: resolvedTotalBytes,
              logMessage:
                  'Downloaded ${_formatBytes(receivedBytes)} of ${_formatBytes(resolvedTotalBytes)}',
            );
          } else {
            downloadFeedNotifier.updateProgress(
              id: transferId,
      ...
      if (bytes.length > 100 * 1024 * 1024) {
        ...
        downloadFeedNotifier.fail(
          transferId,
          'Skipped because it exceeds the 100 MB limit',
        );
        return DriveCacheEntry(
          path: '',
          uuid: '',
          bytes: 0,
          note:
              'Skipped ${file.name ?? 'file'} because it exceeds the 100 MB limit',
          sourceFile: effectiveFile,
          fingerprint: fingerprint,
        );
      }
      ...
      downloadFeedNotifier.appendLog(
        transferId,
        'Cached file at ${p.basename(cachePath)}',
      );
      ...
      downloadFeedNotifier.fail(
        transferId,
        'Skipped because it could not be downloaded',
      );
      _showCompletionNotification(title: fileLabel, body: 'Download failed');
      log(
        'DriveListingView: unexpected download failure',
        error: error,
        stackTrace: stackTrace,
      );
      ...
      return DriveCacheEntry(
        ...
        bytes: 0,
        note:
            'Skipped ${file.name ?? 'file'} because it could not be downloaded',
        sourceFile: effectiveFile,
        fingerprint: fingerprint,
      );
    } finally {
  ...
  void _showDownloadProgressNotification({
    ...
    try {
      ...
      final progressValue = hasTotal
          ? receivedBytes.clamp(0, totalBytes)
  ...
  Future<({List<DriveCacheEntry> entries, List<String> notes})>
  _downloadFolderAsFiles({
    ...
    if (folderId == null) {
      return (
        entries: <DriveCacheEntry>[],
        notes: <String>['Folder missing id'],
      );
    }

    final children = await drive_service.DriveBrowser.listFolderContents(
      folderId,
      apiKey: apiKey,
    );
  ...
  Future<void> _annotateImportedDriveContents({
    ...
    final limit = entries.length < results.length
        ? entries.length
        : results.length;
    ...
    for (var index = 0; index < limit; index++) {
      ...
      final metadata = content.metadata?.copyWith(
        originalFileName:
            ...
            entries[index].fingerprint.sourceName,
      ...
      await ModuleContentRepo.add(
        content.copyWith(xxh3Hash: content.xxh3Hash, metadata: metadata),
      );
    }
  }

  void _showDriveOperationFeedback({
    required DriveImportOutcome outcome,
    required String successLabel,
  }) {
    if (outcome.note != null && outcome.note!.isNotEmpty) {
      final countLabel = outcome.itemCount == 1
          ? '1 item'
          : '${outcome.itemCount} items';
      final message = outcome.itemCount > 0
          ? '$successLabel $countLabel with warnings. ${outcome.note!}'
          : outcome.note!;
      _showCompletionNotification(
        title: outcome.itemCount > 0
      ...
      _showDriveMessage(
        message,
        outcome.itemCount > 0 ? FlushbarVibe.warning : FlushbarVibe.error,
      );
      return;
    }
    ...
    _showCompletionNotification(
      ...
      body: '${outcome.title}: $successLabel $countLabel',
    );
  ...
  Future<Module?> _resolveOrCreateImportCollection(
    String courseId,
    String title,
  ) async {
    final existing = await ModuleRepo.getByTitleAndParentId(
      title: title,
      parentId: courseId,
    );
    if (existing != null) {
    ...
    if (result != null) {
      return await ModuleRepo.getByTitleAndParentId(
        title: title,
        parentId: courseId,
      );
    }
  ...
  SyncType _resolveHistoryType(drive_service.DriveResource resource) {
    ...
    if (file != null) {
      ...
      if (file.isGoogleNative ||
          (file.mimeType ?? '').toLowerCase().contains('document')) {
        return SyncType.course;
      }
    }
  ...
  Future<void> _openInApp(
    ...
    try {
      final resolvedFile =
          preResolvedFile ?? await _resolveDriveDownloadSource(file, apiKey);
      if (resolvedFile == null) {
        _showDriveMessage(
          'Could not resolve the shortcut target',
          FlushbarVibe.error,
        );
        return;
      }
      ...
      if (onlineUrl == null) {
        _showDriveMessage(
          'Could not resolve a preview URL for this file',
          FlushbarVibe.error,
        );
        return;
      }
          ...
          MaterialPageRoute(
            builder: (_) => OnlineViewer(
              args: OnlineViewerArgs(
                title:
                    resolvedFile.name ?? p.basenameWithoutExtension(onlineUrl),
                fallbackUrl: onlineUrl,
                kind: openKind == _DriveInAppOpenKind.pdf
                    ? OnlineViewerKind.pdf
                    : OnlineViewerKind.image,
              ),
            ),
          ),
        );
      ...
      log(
        'DriveListingView: open in app failed',
        error: error,
        stackTrace: stackTrace,
      );
  ...
  List<drive_service.DriveFile> _selectedFiles(
    drive_service.DriveResource resource,
    ...
    if (resource.isFolder) {
      return resource.children
              ?.where(
                (file) => file.id != null && selectedIds.contains(file.id),
              )
              .toList() ??
          [];
    }
  ...
  Future<drive_service.DriveFile?> _resolveDriveDownloadSource(
    drive_service.DriveFile file,
    String apiKey,
  ) async {
    if (!file.isShortcut) {
    ...
    try {
      return await drive_service.DriveBrowser.getFileMetadata(
        file.shortcutTargetId!,
        apiKey: apiKey,
      );
    } catch (error, stackTrace) {
      log(
        'DriveListingView: failed to resolve shortcut target',
        error: error,
        stackTrace: stackTrace,
      );
      return null;
  ...
  Future<drive_service.DriveFile> _resolveFileForOpenOptions(
    drive_service.DriveFile file,
  ) async {
  ...
  _DriveInAppOpenKind? _resolveInAppOpenKind(drive_service.DriveFile file) {
    final mimeType = (file.mimeType ?? '').toLowerCase();
    final extension = (file.fileExtension ?? p.extension(file.name ?? ''))
        .replaceFirst('.', '')
        .toLowerCase();
  ...
  String _downloadFileName(drive_service.DriveFile file) {
    ...
    if (extension != null &&
        ...
        !resolved.toLowerCase().endsWith('.$extension')) {
...
final driveListingControllerProvider = Provider.autoDispose
    .family<DriveListingController, String>((ref, collectionId) {

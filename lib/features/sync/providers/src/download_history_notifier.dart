...
class DownloadHistoryNotifier
    extends
        HiveAsyncImpliedNotifierN<List<dynamic>, List<Map<String, dynamic>>> {
  DownloadHistoryNotifier()
    : super(
        HiveDataKey.downloadHistory.name,
        defaultKey: const [],
        builder: (data) {
          if (data is List) {
            return data
  ...
  Future<void> addEntry(DownloadHistoryEntry entry) async {
    final current = List<Map<String, dynamic>>.from(
      state.value ?? const <Map<String, dynamic>>[],
    );
...
final downloadHistoryFilterProvider = StateProvider.autoDispose<SyncType?>(
  (ref) => null,
);

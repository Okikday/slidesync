...
final _settingsProvider = AsyncNotifierProvider.autoDispose(
  () => HiveAsyncImpliedNotifier<Map, SettingsModel>(
    HiveDataKey.isBuiltInViewer.name,
    const SettingsModel(),
    isUpdateNotifying: true,
    transformer: (p0) => p0.toMap(),
    builder: (data) => data == null
        ? const SettingsModel()
        : SettingsModel.fromMap(Map.castFrom(data)),
  ),
);
...
class SettingsProvider {
  ...
  static final state =
      NotifierProvider<SettingsRevealNotifier, SettingsRevealState>(
...
extension SettingsModelExtension on WidgetRef {
  Future<SettingsModel> get readSettings async =>
      await read(_settingsProvider.future);
}

...
class ModifyContentUc {
  ...
  Future<String?> deleteContent(ModuleContent content) async {
    final dupHashExists = await ModuleContentRepo.doesDuplicateHashExists(
      content.xxh3Hash,
    );
    ...
    if (!dupHashExists) {
      ...
      await FileUtils.deleteFileAtPath(
        content.metadata?.thumbnail?.local ?? '',
      );
    }
    if (!dupHashExists) {
  ...
  static Future<bool> removeIdFromRecents(String contentId) async {
    final result = (await Result.tryRunAsync(() async {
      ...
      final rawOldRecents =
          (await KVStore.me.getData(key: HiveDataKey.recentContentsIds.name))
              as List<String>?;
      if (rawOldRecents == null) {
        ...
        if (recents.remove(contentId)) {
          await KVStore.me.setData(
            key: HiveDataKey.recentContentsIds.name,
            value: recents.toList(),
          );
          return true;
        }
        return false;
  ...
  Future<String?> renameContent(ModuleContent content, String newTitle) async {
    return (await Result.tryRunAsync(() async {
      ...
      await ModuleContentRepo.add(
        stContent.copyWith(xxh3Hash: content.xxh3Hash, title: newTitle),
      );
      try {

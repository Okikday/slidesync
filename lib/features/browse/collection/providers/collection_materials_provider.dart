...
class CollectionMaterialsProvider {
  ...
  static final _contentPaginationFutureProvider = FutureProvider.autoDispose
      .family<CollectionMaterialsPagination, String>((ref, collectionId) async {
        final sortOption = (await ref.read(contentSortOptionProvider.future));
        final cp = CollectionMaterialsPagination.of(collectionId, sortOption: sortOption);
        ref.listen(_watchContentChanges(collectionId), (prev, next) async => await compareContentAndUpdate(cp));
        ref.onDispose(() => cp.dispose());
        return cp;
      });

  static FutureProvider<CollectionMaterialsPagination> contentPaginationProvider(String collectionId) =>
